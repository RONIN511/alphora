...
class OpenAILike(BaseLLM):
    ...
    async def aget_streaming_response_dynamic(
            self,
            message: Union[str, Message, List[Dict[str, Any]]],
            content_type: str = "char",
            enable_thinking: bool = False,
        ...
        """
        异步 - 流式输出 (支持 Tools)
        - content_type 动态取 delta.content_type（若无则回退到入参 content_type）
        - 忽略 delta.content_type == 'stop' 的 chunk（不 yield）
        """
        tracer = self._get_tracer()
        call_id = None
        ...
        class AsyncStreamGenerator(BaseGenerator[GeneratorOutput]):
            def __init__(self, async_stream, default_content_type: str, tracer_ref, call_id_ref, agent_id_ref):
            ...
            async def agenerate(self) -> AsyncIterator[GeneratorOutput]:
                try:
                    async for chunk in self._stream:
                        ...
                        # 动态 content_type：优先 delta.content_type，否则回退到初始化的 self.content_type
                        delta_ct = getattr(delta, "content_type", None) or self.content_type

                        # 忽略 stop 类型（不 yield；reasoning 仍按你原逻辑优先输出）
                        if delta_ct == "stop":
                            continue
                        ...
                        if reasoning:
                            ...
                            if self._tracer and self._call_id:
                                self._tracer.track_llm_stream_chunk(
                                    self._call_id, content, delta_ct, is_reasoning=False
                                )
                            yield GeneratorOutput(content=content, content_type=delta_ct)
        ...
        return AsyncStreamGenerator(stream, content_type, tracer, call_id, self.agent_id)
